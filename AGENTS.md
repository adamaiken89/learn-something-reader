...
- **FSRS-6**: spaced repetition algo in `src/bun/fsrs.ts` (21-param weight array, `W[4]=6.4133` is the only FSRS-5→6 delta). `nextInterval` hardcodes `0.9` retention (FSRS-6 default). Schema carries `stability`/`difficulty`/`lapses`/`state` per card; no migration code — older decks keep FSRS-5 numbers but new reviews compute against v6 weights. No per-card `desired_retention` (deferred).
- **TS7 via native**: `@typescript/native@npm:typescript@^7.0.2` (Go `tsc` binary) is the only TypeScript — old `typescript@6` JS-API pin removed. `.bin/tsc` resolves to TS7 via the alias. `tsconfig.json` needs `"types": ["*"]` (TS7 default is `[]`). Bun alias resolution differs from node — use direct installs, not `@typescript/old`.
- **Electrobun 2.x (Hutch)**: `electrobun@2.0.1` pinned exact in **devDependencies** (dependency-free bootstrap; first CLI call downloads Hutch core to `~/.hutch`). **Fresh clone MUST run `bunx electrobun sync` before test/build** — materializes SDK into `.hutch/devkit/` (gitignored). Config: `electrobun.config.ts` (`build.mainProcess:"bun"` + entrypoint) + `hutch.config.ts` (`{packageManager:"bun"}`). Backend imports `electrobun/main` (not deprecated `electrobun/bun`).
- **Electroview NO-ALIAS design**: `src/mainview/rpc.ts` imports `Electroview` directly from `'../../.hutch/devkit/api/browser/index'`. Do NOT introduce vite/tsconfig aliases for it — vite's internal alias plugin resolves BEFORE any `resolveId` hook (even `enforce:'pre'`), which shadows the e2e mock-view plugin and silently breaks all e2e tests. tsconfig `paths` map `electrobun*` → devkit entries; `lib: ES2022`; no `baseUrl` (removed in TS7).
- **Devkit mock seam**: `setup.tsx` `mock.module` targets BOTH the absolute path and `file://` URL of `.hutch/devkit/api/browser/index.ts` (resolved via `fileURLToPath(import.meta.url)`); `vite.config.ts` mockViewPlugin matches ids ending in that same path. Changing either side alone = silent mock bypass.
...
- **Prefer value assertions over boolean ones**. `expect(state).toBe('lesson')` not `toBe(true)`, `expect(arr).toHaveLength(3)` not `toBeTruthy()`, `expect(node).toHaveAttribute('data-x', 'y')` not `toBe(true)`. `toBe(true|false)`/`toBeTruthy()`/`toBeFalsy()` accepted ONLY when: (a) checking DOM presence (`querySelector` returns `Element|null` — value compare would be `!== null`, same signal, but truthy reads cleaner here), (b) boolean return from store getter (`isSyncing`, `loaded`, `hasAnswer`). For comparisons of booleans produced by other expressions (e.g. `a < b`, `mockResponses.has(k)`, `views.some(...)`, `classList.contains(...)`) prefer `toBe(true)` over wrapping in `toBeTruthy()` — explicit boolean reads as a checked assertion, not a smoke test. When refactoring legacy `toBe(true)` on numeric/string state, do that as part of the same commit that touches the file, don't open a standalone cleanup pass.
...
- **Coverage gate**: `bun run coverage` = full suite + coverage report, gated by bunfig `coverageThreshold = { functions = 0.7, lines = 0.7 }`. Bun enforces the threshold **per-file** (any single file below → exit 1), NOT on the aggregate — floor values are current repo minimums; ratchet up as legacy files gain tests. `coverageSkipTestFiles = true` is REQUIRED: without it every `.test.ts(x)` counts as 0% and the per-file gate can never pass. UI-heavy components intentionally below the line are covered by Playwright e2e instead.
- **Coverage ignore list** (bunfig `coveragePathIgnorePatterns`): shadcn primitives + UI shells covered by e2e/mocked deps that can't reach inner components:
  - `src/mainview/components/lesson/LessonContentViewer.tsx` — PerplexityButton/ClozeBlockquote only reachable via real `react-markdown` (mocked in tests). Covered by Playwright e2e.
  - `src/mainview/components/shadcn/sheet.tsx` — `SheetHeader/Footer/Description` only consumed by the BottomSheet, not exported for direct unit test. Covered via `bottom-sheet.test.tsx`.
