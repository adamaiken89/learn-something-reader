...
- **Lint/format tooling**: oxlint + oxfmt replace ESLint + Prettier. Config: `.oxlintrc.json` (typeAware on via `options.typeAware`, type-aware rules need `oxlint-tsgolint` pkg; react-compiler + simple-import-sort run through `jsPlugins` bridge — those 2 npm packages must stay installed), `.oxfmtrc.json` (prettier-migrated). Scripts: `lint` (oxlint+oxfmt check, used by husky pre-commit), `fix`, `check` (tsc + lint). Type-aware probe gates all verified firing: no-floating-promises, no-misused-promises, prefer-nullish-coalescing (`ignorePrimitives` object form `{string:true,boolean:true}` = old array semantics), import/no-cycle (needs `"import"` in ROOT plugins array, not override-only), simple-import-sort, react-compiler.
